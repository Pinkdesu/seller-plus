...
import { getUserReport, resetReport, getOrderPeriod } from '../store/events';
...
const UserReport = () => {
  ...
  const pending = useStore(getUserReport.pending);
  ...
  const handleClick = () => {
    getUserReport({ periodFrom, periodTo });
  };
  ...
  return (
    <div className={classes.root}>
      <Header title="Отчёт по количеству заказов у пользователя" />
      <Container className={classes.formWrapper}>
        ...
        <div className={classes.tableWrapper}>
          {Boolean(report.length) && (
            <DataTable columns={USER_COLUMNS} data={report} />
          )}
        </div>
      </Container>
    </div>
